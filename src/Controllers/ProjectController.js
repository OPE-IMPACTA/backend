...
class ProjectController extends BaseController {
  ...
  async store (request, response) {
    try {
      ...
      HttpHelper.response(response, 200, {
        _id,
        cnpj,
        company,
        department,
  ...
  async summary (request, response) {
    try {
      ...
      let resultFormatted = { all: 0 }
      result.forEach(({ _id: id, count }) => {
        resultFormatted[id] = count
        resultFormatted.all += count
      })

      HttpHelper.response(response, 200, resultFormatted)
    } catch (error) {
      HttpHelper.response(response, 500, [], 'Houve um erro, tente mais tarde!')
    }
  }
